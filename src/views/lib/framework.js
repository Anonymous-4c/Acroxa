// ../src/views/lib/framework.js
// framework.js

// List of HTML void elements (self-closing, no content or closing tag allowed)
const VOID_ELEMENTS = new Set([
  'area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input',
  'link', 'meta', 'param', 'source', 'track', 'wbr',
  // Common SVG elements used in HTML context that behave as void
...
function em(attrs = {}, ...children) {
  return el('em', attrs, ...children);
}

function code(attrs = {}, ...children) {
  return el('code', attrs, ...children);
}
...
function blockquote(attrs = {}, ...children) {
  return el('blockquote', attrs, ...children);
}
...
function figcaption(attrs = {}, ...children) {
  return el('figcaption', attrs, ...children);
}
...
function PageHeader({ title, actionText = 'Create New', actionHref = '#', actionIcon = 'add', extraClasses = '' }) {
  return el('div', { class: `main-head d-flex flex-row ${extraClasses}`.trim() },
    ...
    el('button', {
      ...
      'data-': actionHref   // using data- attribute as in your code
    },
      icon(actionIcon, 'solid'),
      el('span', {}, actionText)
    )
  );
}
...
function PaginationControls() {
  return el('div', { class: 'pg-fdpv paginator-controls' },
    el('button', { class: 'p-2 button-pst prev' },
      icon('chevron-left', 'solid')
    ),
    el('span', { class: 'page-info' }, ''),   // will be filled by JS
    el('button', { class: 'p-2 button-pst next' },
...
function StatusTabs({ active = 'all' } = {}) {
  ...
  return el('div', { class: 'links' },
    ...items.flatMap((item, i) => [
      el('div', {
        class: `pages ${item.id === active ? 'active' : ''}`,
        'data-show-sid': item.id
      }, item.label),
...
function PostsPerPageDropdown({ current = 10, name = 'postsPerPage' } = {}) {
  ...
  return el('div', { class: 'dropdown num-exp pp-dropdown'},
    el('button', { type: 'button', class: 'dropdown-toggle pp-dropdown-toggle' },
      el('span', { class: 'dull' }, 'Posts'),
      el('span', { class: 'p-n-m' }, current),
      icon('chevron-down', 'solid')
    ),
    el('input', {
      type: 'hidden',
    ...
    el('div', { class: 'dropdown-menu pp-dropdown-menu' },
      el('div', { class: 'wrap-menu-dp' },
        ...options.map(val =>
          el('button', {
            class: 'dropdown-item pp-dropdown-item',
            'data-value': val,
            ...(val === current ? { 'data-active': 'true' } : {})
          }, val)
        )
      )
    )
...
// ─── New: Generic Custom Dropdown ───────────────────────────────────────────────

// ─── New: Category Filter Dropdown (preset) ─────────────────────────────────────
...
function PostsTableSkeleton() {
  return el('div', { class: 'posts-ttc-wrap acrx-ttc-wrap' },
    el('div', { class: 'posts-ttc acrx-ttc' },
      ...
      // Body placeholder (rows added by JS later)
      el('div', { class: 'ttc-pis ttc-pis-wrap' })
    )
  );
}

// ─── New: Posts Page Main Content Wrapper ───────────────────────────────────────
function PostsMainContent(...children) {
  return el('div', { class: 'dshb-content' }, ...children);
  // You can change class name if you prefer something else (posts-content, etc.)
}
// ─── Helper: Simple Input ───────────────────────────────────────────────
...
// ─── Updated / more flexible MediaHeader ────────────────────────────────
// (replaces or extends previous MainHeader when needed)
  ...
  return el('div', { class: 'media-header main-head' },
    ...
    el('div', { class: 'media-actions' },
      ...
      Input({
        ...
        className: ''   // add classes if needed
      })
    )
  );
}
...
function MediaGridPlaceholder() {
  return el('div', { class: 'media-grid', id: 'media-grid' },
    // comment or small placeholder text if desired
...
function Dropdown({ id, options = [], value = '', extraClass = '' } = {}) {
  ...
  const currentLabel = currentOpt
    ? (typeof currentOpt === 'object' ? currentOpt.label : currentOpt)
    : (options[0] ? (typeof options[0] === 'object' ? options[0].label : options[0]) : 'Select…');
  ...
  return el('div', { class: 'dropdown-field-wrap', 'data-dropdown-id': id },
    CustomDropdown({
      ...
      items: options,
      valueAttr: 'data-value',
      extraClass: `settings-dropdown ${extraClass}`.trim(),
      id: `dd-${id}`,
      name: id,
      value,
    })
...
function SaveBar({ id = 'save-btn' } = {}) {
  return el('div', { class: 'save-bar form-actions' },
    el('button', { id, class: 'btn-save btn ghost btn-lg' },
      _Icon('floppy-disk', 'solid'),
...
function Badge(text, variant = 'default') {
  return el('span', { class: `badge badge-${variant} badge-pill` }, text);
}
...
function DangerZone(...children) {
  return el('div', { class: 'danger-zone danger-card' },
    el('div', { class: 'danger-header' },
      _Icon('triangle-exclamation', 'solid'),
      el('h3', { class: 'danger-title' }, 'Danger Zone')
    ),
    ...children
  );
}
...
function Callout({ type = 'info', title, message } = {}) {
  ...
  return el('div', { class: `callout callout-${type} callout-box` },
    ...
    el('div', { class: 'callout-content' },
      title ? el('strong', { class: 'callout-title' }, title) : null,
      message ? el('p', { class: 'callout-message' }, message) : null
    )
  );
}
